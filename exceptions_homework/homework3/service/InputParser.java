package exceptions_homework.homework3.service;
...
import service.exceptions.EmptyInputException;
import service.exceptions.InputNotEnoughValuesException;
import service.exceptions.InputTooManyValuesException;
...
public class InputParser {
    ...
    private static boolean checkForPhoneAndSaveValueIfTrue(@NotNull String s) throws IOException {
        try {
            ...
            if (phoneLength[0] <= String.valueOf(phone).length() && String.valueOf(phone).length() <= phoneLength[1]) {
                ...
                throw new IOException("Вы ввели '" + s + "' в качестве телефона. " +
    ...
    private static boolean checkForNameAndSaveValueIfTrue(String s) throws IOException {
        ...
        if (parsedInputHashMap.get(keys[0]) == null) {
            ...
            if (lang == currentLang) {
                ...
                throw new IOException("Введенный язык имени не совпадает с языком фамилии.");
            ...
            if (lang == currentLang) {
                ...
                throw new IOException("Введенный язык отчества не совпадает с языком фамилии.");
    ...
    private static int whatLanguage(@NotNull String s) throws IOException {
        if (isCyrillic(s.replaceAll("\\p{P}", ""))) {
            ...
            throw new IOException("Введенный язык не распознан.");
    }
    ...
    private static String stringFilterWithHyphensAndCapitalize(@NotNull String s) throws IOException {
        ...
        // (&&) ->
        // -> заменим все знаки пунктуации [ \p{P} ] кроме слов с дефисом внутри [
        // \\w+(?:-\\w+)+ ]
        ...
        // Если да -> то распарсим его, капитализируем каждое имя в отдельности и вновь
        // склеим через дефис
        try {
